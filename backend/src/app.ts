...
const simplifyText = async (text: string, userGroup: TargetAudiences, outputLanguage: string, apiKey: string): Promise<string> => {
  const instructions = `Do no write long sentences.
  If the provided text is a URL, you need to visit the URL, summarise the contents, 
  and finally simplify the summary into plain language.
  If I provide you with a language, you need to first simplify the text into plain language and then translate it into the provided language.
...
app.post('/simplify', upload.single('file'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const simplifiedText = await simplifyText(inputText, audience, outputLanguage, apiKey);
